...
def build_dataset(result_path, mapping_path, sol_path=None, contracts_dir=None):
    ...
    for item in result["analysis"]:
        ...
        for el in func_elements:
            ...
            functions[func_key]["labels"].add(label)
            functions[func_key]["checks"].add(check)
    ...
    for f in functions.values():
        # Convert "Ignore" label to "No Vulnerability" only if no other real vulnerabilities
        labels = list(f["labels"])
        real_vulnerabilities = [lbl for lbl in labels if lbl not in ("Ignore", "Other")]
        
        if real_vulnerabilities:
            # Keep only real vulnerabilities
            labels = real_vulnerabilities
        elif "Ignore" in labels or "Other" in labels:
            # No real vulnerability found, mark as "No Vulnerability"
            labels = ["No Vulnerability"]
